...
resource "aws_cognito_user_pool" "fss_pool" {
    ...
    email_configuration {
        ...
        source_arn             = "arn:aws:ses:eu-west-1:859159924354:identity/fss@hackney.gov.uk"
    }
...
resource "aws_cognito_user_pool_domain" "fss_domain_development" {
    domain       = "fss-pool-domain-development"
    user_pool_id = aws_cognito_user_pool.fss_pool.id
}
